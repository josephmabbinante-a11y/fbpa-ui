...
const EXAMPLE_LANES = [
  { origin: 'Los Angeles, CA', destination: 'Dallas, TX', miles: 1430, rpm: 2.15 },
  { origin: 'Chicago, IL', destination: 'Miami, FL', miles: 1380, rpm: 2.42 },
  { origin: 'Atlanta, GA', destination: 'New York, NY', miles: 880, rpm: 2.68 },
  { origin: 'Denver, CO', destination: 'Phoenix, AZ', miles: 600, rpm: 1.95 },
  { origin: 'Seattle, WA', destination: 'Los Angeles, CA', miles: 1140, rpm: 2.31 },
];
...
export default function TruckloadRateCalculator() {
  ...
  function applyExampleLane(lane) {
    setOrigin(lane.origin);
    setDestination(lane.destination);
    setMiles(lane.miles);
    setMarketRpm(lane.rpm);
  }
  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        <div style={{ fontSize: 28, fontWeight: 700, marginBottom: 6 }}>💰 Freight Pricing & Rate Calculator</div>
        <div style={{ fontSize: 14, color: t.textSecondary }}>
          Real-time freight rate optimization • Carrier buy rates • Broker margin targeting • Win probability
        </div>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        <button style={tabStyle(activeTab === 'calculator')} onClick={() => setActiveTab('calculator')}>Rate Calculator</button>
        <button style={tabStyle(activeTab === 'engine')} onClick={() => setActiveTab('engine')}>Rate Logic Engine</button>
        <button style={tabStyle(activeTab === 'formula')} onClick={() => setActiveTab('formula')}>Pricing Formula</button>
      </div>
      ...
      {activeTab === 'calculator' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={cardStyle}>
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>🚀 Example Lanes — Quick Load</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {EXAMPLE_LANES.map((lane) => (
                  <button
                    key={lane.origin + lane.destination}
                    onClick={() => applyExampleLane(lane)}
                    style={{
                      display: 'flex',
                      ...
                      borderRadius: 4,
                      border: `1px solid ${t.border}`,
                      backgroundColor: origin === lane.origin && destination === lane.destination ? t.bgAlt : 'transparent',
                      color: t.text,
                      cursor: 'pointer',
                      fontSize: 12,
                      textAlign: 'left',
                  ...
                  >
                    ...
                    <span style={{ color: t.textSecondary }}>{lane.miles} mi · ${lane.rpm}/mi</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      {activeTab === 'engine' && (
        <div style={cardStyle}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>🔧 Rate Logic Engine</div>
          <div style={{ fontSize: 13, color: t.textSecondary, marginBottom: 16 }}>
            Advanced rate calculation with lane history harvesting, market model scoring, and bid network integration.
          </div>
          <RateCalculator />
        </div>
      )}
