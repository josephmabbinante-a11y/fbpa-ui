...
router.get('/', async (req, res) => {
  try {
    ...
    const spendByCarrier = Object.entries(
      invoices
      ...
      .map(([carrier, spend]) => ({ carrier, spend: Number(spend.toFixed(2)) }))
      .sort((a, b) => b.spend - a.spend)
      .slice(0, 10);

    const overchargeFrequency = exceptionBreakdown.map((row) => ({
      reason: row.reason,
      count: row.count,
    }));

    const auditRecoveryAmount = Number(topSavingsCarriers.reduce((sum, row) => sum + row.total, 0).toFixed(2));
