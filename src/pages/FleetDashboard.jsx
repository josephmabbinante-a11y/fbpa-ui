  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
      ...
      {activePage === 'drivers' && (
        <section style={{ ...sectionCardStyle, display: 'grid', gap: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontSize: 17, fontWeight: 700 }}>Driver Performance Intelligence</div>
              <div style={{ fontSize: 12, color: t.textSecondary }}>Sortable by profitability, fuel burn, risk, and compliance</div>
            </div>
            <select value={driverSortBy} onChange={(event) => setDriverSortBy(event.target.value)} style={{ borderRadius: 8, border: `1px solid ${t.border}`, background: t.bgAlt, color: t.text, padding: '8px 10px' }}>
              <option value="profitability">Sort: Profitability</option>
              <option value="fuel">Sort: Fuel burn</option>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {leaderboardRows.map((driver) => (
              <div key={driver.id} style={{ border: `1px solid ${t.border}`, borderRadius: 10, background: t.bgAlt, padding: 10, display: 'grid', gap: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
                  <div style={{ fontWeight: 700 }}>{driver.name}</div>
                  <div style={{ fontSize: 12, color: t.textSecondary }}>
                    Profitability {driver.profitabilityScore} • RPM ${Number(driver.revenuePerMile).toFixed(2)} • On-time {driver.onTimePct}%
                  </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0,1fr))', gap: 8, fontSize: 12 }}>
                  <div>Idle time: <strong>{driver.idlePct}%</strong></div>
                  <div>Fuel variance: <strong>{driver.fuelDeltaPct > 0 ? '+' : ''}{driver.fuelDeltaPct}%</strong></div>
                  <div>Safety score: <strong>{driver.safetyScore}</strong></div>
                  <div>Risk score: <strong>{driver.riskScore}</strong></div>
                  <div>Compliance: <strong>{driver.complianceScore}</strong></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 6 }}>
                  {driver.efficiencyTrend.map((point, idx) => (
                    <div key={`${driver.id}-trend-${idx}`} style={{ display: 'grid', gap: 4 }}>
                      <div style={{ fontSize: 10, color: t.textSecondary }}>W{idx + 1}</div>
                      <ProgressBar value={point} color={point > 82 ? t.success : point > 68 ? t.warning : t.error} t={t} />
                    </div>
                  ))}
                </div>
              </div>
      ...
      {activePage === 'financial' && (
        <section style={{ ...sectionCardStyle, display: 'grid', gap: 12 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700 }}>Financial Ops</div>
            <div style={{ fontSize: 12, color: t.textSecondary }}>Broker-intelligent margin and cost control module</div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 8 }}>
            <div style={{ border: `1px solid ${t.border}`, borderRadius: 8, background: t.bgAlt, padding: 10 }}>
              <div style={{ fontSize: 12, color: t.textSecondary }}>Today Margin</div>
              <div style={{ fontSize: 26, fontWeight: 800 }}>{formatMoney(financialSnapshot.margin)}</div>
            </div>
            <div style={{ border: `1px solid ${t.border}`, borderRadius: 8, background: t.bgAlt, padding: 10 }}>
              <div style={{ fontSize: 12, color: t.textSecondary }}>Projected Weekly Burn</div>
              <div style={{ fontSize: 26, fontWeight: 800, color: projectedWeeklyBurn > financialSnapshot.projectedWeeklyGross ? t.error : t.warning }}>{formatMoney(projectedWeeklyBurn)}</div>
            </div>
            <div style={{ border: `1px solid ${t.border}`, borderRadius: 8, background: t.bgAlt, padding: 10 }}>
              <div style={{ fontSize: 12, color: t.textSecondary }}>Forecasted Maintenance Expense</div>
              <div style={{ fontSize: 26, fontWeight: 800 }}>{formatMoney(forecastedMaintenanceExpense)}</div>
            </div>
            <div style={{ border: `1px solid ${t.border}`, borderRadius: 8, background: t.bgAlt, padding: 10 }}>
              <div style={{ fontSize: 12, color: t.textSecondary }}>Revenue at Risk</div>
              <div style={{ fontSize: 26, fontWeight: 800, color: revenueAtRiskToday > 2600 ? t.error : revenueAtRiskToday > 1200 ? t.warning : t.success }}>{formatMoney(revenueAtRiskToday)}</div>
            </div>
          </div>

          <div style={{ border: `1px solid ${t.border}`, borderRadius: 8, background: t.bgAlt, padding: 10 }}>
            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 6 }}>Weekly Revenue Trend</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))', gap: 6 }}>
              {weeklyRevenueTrend.map((value, index) => {
                const height = clamp(Math.round((value / Math.max(...weeklyRevenueTrend, 1)) * 100), 20, 100);
                return (
                  <div key={`wk-${index}`} style={{ display: 'grid', gap: 4, alignItems: 'end' }}>
                    <div style={{ height: 72, display: 'flex', alignItems: 'flex-end' }}>
                      <div style={{ width: '100%', height: `${height}%`, borderRadius: 6, background: t.accent, border: `1px solid ${t.border}` }} />
                    </div>
                    <div style={{ fontSize: 10, color: t.textSecondary }}>D{index + 1}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ overflowX: 'auto', border: `1px solid ${t.border}`, borderRadius: 8 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '1rem' }}>
              <thead style={{ background: t.bgAlt, color: t.textSecondary }}>
                <tr>
                  <th style={{ padding: 8, textAlign: 'left' }}>Lane</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Units</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Revenue</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Cost</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Margin</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Margin %</th>
              ...
              <tbody>
                {laneMargins.map((row) => (
                  <tr key={row.lane} style={{ borderTop: `1px solid ${t.border}` }}>
                    <td style={{ padding: 8 }}>{row.lane}</td>
                    <td style={{ padding: 8, textAlign: 'right' }}>{row.units}</td>
                    <td style={{ padding: 8, textAlign: 'right' }}>{formatMoney(row.revenue)}</td>
                    <td style={{ padding: 8, textAlign: 'right' }}>{formatMoney(row.cost)}</td>
                    <td style={{ padding: 8, textAlign: 'right', color: row.margin < 0 ? t.error : t.success }}>{formatMoney(row.margin)}</td>
                    <td style={{ padding: 8, textAlign: 'right', color: row.marginPct < 12 ? t.warning : t.success }}>{row.marginPct.toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {activePage === 'maintenance' && (
        <section style={{ ...sectionCardStyle, display: 'grid', gap: 12 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700 }}>Maintenance Command</div>
            <div style={{ fontSize: 12, color: t.textSecondary }}>Downtime is visualized as direct cost to protect margin</div>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '1rem' }}>
              <thead style={{ background: t.bgAlt, color: t.textSecondary }}>
                <tr>
                  <th style={{ padding: 8, textAlign: 'left' }}>Unit</th>
                  <th style={{ padding: 8, textAlign: 'left' }}>Status</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Time in Repair</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Parts ETA</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Cost vs Budget</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Technician Load</th>
                  <th style={{ padding: 8, textAlign: 'right' }}>Downtime Cost</th>
                  <th style={{ padding: 8, textAlign: 'left' }}>Return to Service</th>
                  <th style={{ padding: 8, textAlign: 'left' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {maintenanceCommandRows.map((row) => (
                  <tr key={row.id} style={{ borderTop: `1px solid ${t.border}` }}>
                    <td style={{ padding: 8 }}>{row.unitNumber}</td>
                    <td style={{ padding: 8 }}>
                      <span style={{ borderRadius: 999, border: `1px solid ${toneColor(t, statusTone(row.status))}`, color: toneColor(t, statusTone(row.status)), padding: '2px 8px' }}>{row.status}</span>
                    </td>
                    <td style={{ padding: 8, textAlign: 'right' }}>{row.timeInRepairHours}h</td>
                    <td style={{ padding: 8, textAlign: 'right' }}>{row.partsEtaHours}h</td>
                    <td style={{ padding: 8, textAlign: 'right', color: row.costVsBudget > 0 ? t.error : t.success }}>{formatMoney(row.costVsBudget)}</td>
                    <td style={{ padding: 8, textAlign: 'right' }}>{row.techLoad}</td>
                    <td style={{ padding: 8, textAlign: 'right', color: row.downtimeCostPerUnit > 800 ? t.error : t.warning }}>{formatMoney(row.downtimeCostPerUnit)}</td>
                    <td style={{ padding: 8 }}>{new Date(row.estimatedReturnToService).toLocaleString()}</td>
                    <td style={{ padding: 8 }}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        <button type="button" onClick={() => assignTechnician(row.id)} style={{ borderRadius: 6, border: `1px solid ${t.border}`, background: t.bgAlt, color: t.text, cursor: 'pointer' }}>Assign tech</button>
                        <button type="button" onClick={() => moveToYard(row.id)} style={{ borderRadius: 6, border: `1px solid ${t.border}`, background: t.bgAlt, color: t.text, cursor: 'pointer' }}>Move yard</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {activePage === 'telematics' && (
        <section style={{ ...sectionCardStyle, display: 'grid', gap: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontSize: 17, fontWeight: 700 }}>Live Telematics Tactical View</div>
              <div style={{ fontSize: 12, color: t.textSecondary }}>Clustered map with filters and vehicle side drawer</div>
            </div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {['all', 'idle', 'breakdown', 'speeding', 'geofence'].map((status) => (
                <button key={status} type="button" onClick={() => setTelematicsFilter(status)} style={{ borderRadius: 8, border: `1px solid ${telematicsFilter === status ? t.accent : t.border}`, background: t.bgAlt, color: t.text, padding: '5px 9px', cursor: 'pointer' }}>
                  {status}
                </button>
              ))}
              <button type="button" onClick={() => setFullMapMode((prev) => !prev)} style={{ borderRadius: 8, border: `1px solid ${t.border}`, background: t.bgAlt, color: t.text, padding: '5px 9px', cursor: 'pointer' }}>
                {fullMapMode ? 'Exit Full Map' : 'Full Screen Map'}
              </button>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: fullMapMode ? '1fr' : 'minmax(0, 2fr) minmax(280px, 1fr)', gap: 10 }}>
            <div style={{ border: `1px solid ${t.border}`, borderRadius: 10, background: t.bgAlt, minHeight: fullMapMode ? 520 : 360, position: 'relative' }}>
              {telematicsRows.map((vehicle) => {
                const point = mapPoint(vehicle.currentLat, vehicle.currentLng);
                const clusterSize = vehicle.status === 'active' ? 14 : 12;
                return (
                  <button
                    key={vehicle.id}
                    type="button"
                    ...
                    style={{
                      ...
                      top: `${point.y}%`,
                      transform: 'translate(-50%, -50%)',
                      width: selectedVehicleId === vehicle.id ? clusterSize + 4 : clusterSize,
                      height: selectedVehicleId === vehicle.id ? clusterSize + 4 : clusterSize,
                      borderRadius: 999,
                      border: `1px solid ${t.border}`,
                      background: toneColor(t, statusTone(vehicle.status)),
                      cursor: 'pointer',
            ...
            {!fullMapMode && (
              <aside style={{ border: `1px solid ${t.border}`, borderRadius: 10, background: t.bgAlt, padding: 10, display: 'grid', gap: 8, alignContent: 'start' }}>
                <div style={{ fontSize: 14, fontWeight: 700 }}>{selectedTelematicsVehicle?.unitNumber || 'Select vehicle'}</div>
                <div style={{ fontSize: 12 }}>Driver: <strong>{selectedTelematicsVehicle?.driverName || '—'}</strong></div>
                <div style={{ fontSize: 12 }}>Idle duration: <strong>{selectedTelematicsVehicle?.idleMinutes || 0} min</strong></div>
                <div style={{ fontSize: 12 }}>Route deviation alert: <strong>{selectedTelematicsVehicle && selectedTelematicsVehicle.riskScore > 75 ? 'Active' : 'None'}</strong></div>
                <div style={{ fontSize: 12, color: t.textSecondary }}>Last 5 stops</div>
                <div style={{ display: 'grid', gap: 4, fontSize: 12 }}>
                  {[1, 2, 3, 4, 5].map((idx) => (
                    <div key={`stop-${idx}`} style={{ border: `1px solid ${t.border}`, borderRadius: 6, padding: '4px 6px' }}>
                      Stop {idx}: {selectedTelematicsVehicle?.lastTrip || 'Unknown lane'}
                    </div>
                  ))}
                </div>
              </aside>
      ...
      {showMarginDriftModal && (
        <section style={{ ...sectionCardStyle, display: 'grid', gap: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
            <div>
              <div style={{ fontSize: 16, fontWeight: 700 }}>Margin Drift Trend</div>
              <div style={{ fontSize: 12, color: t.textSecondary }}>Actionable drift signal for pricing and load assignment</div>
            </div>
            <button type="button" onClick={() => setShowMarginDriftModal(false)} style={{ borderRadius: 8, border: `1px solid ${t.border}`, background: t.bgAlt, color: t.text, padding: '6px 10px', cursor: 'pointer' }}>
              Close
            </button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, minmax(0,1fr))', gap: 8 }}>
            {[marginDrift - 2.2, marginDrift - 1.3, marginDrift - 0.9, marginDrift - 0.2, marginDrift + 0.4, marginDrift].map((point, idx) => {
              const normalized = clamp(Math.round((point + 6) * 8), 10, 100);
              return (
                <div key={`drift-${idx}`} style={{ display: 'grid', gap: 4 }}>
                  <ProgressBar value={normalized} color={point < -1 ? t.error : point < 1 ? t.warning : t.success} t={t} />
                  <div style={{ fontSize: 11, color: t.textSecondary, textAlign: 'center' }}>{point.toFixed(1)}%</div>
                </div>
              );
            })}
          </div>
