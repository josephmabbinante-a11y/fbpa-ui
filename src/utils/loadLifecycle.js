...
export function deriveRouteDetailModules(status) {
  const normalized = normalizeLoadStatus(status);
  const isDeliveredOrLater = ['DELIVERED', 'POD_RECEIVED', 'INVOICED', 'READY_TO_PAY', 'PAID'].includes(normalized);

  return {
    status: normalized,
    isDeliveredOrLater,
    isInTransit: normalized === 'IN_TRANSIT',
    isPreDispatch: normalized === 'PRE_DISPATCH',
    isException: normalized === 'EXCEPTION',
    showPickupCountdown: normalized === 'PRE_DISPATCH',
    showDispatchControls: normalized === 'PRE_DISPATCH',
    showCapacityHeat: normalized !== 'DELIVERED',
    showPODPanel: isDeliveredOrLater,
    showInvoiceReadiness: isDeliveredOrLater,
    showMarginLockStatus: isDeliveredOrLater,
    showLiveTracking: normalized === 'IN_TRANSIT',
    showSlaDelta: normalized === 'IN_TRANSIT',
    showRiskIndicators: ['IN_TRANSIT', 'EXCEPTION'].includes(normalized),
    showEtaRecalculation: normalized === 'IN_TRANSIT',
    highlightException: normalized === 'EXCEPTION',
    showExceptionReason: normalized === 'EXCEPTION',
    lockInvoice: normalized === 'EXCEPTION',
    showEscalateAlertModule: normalized === 'EXCEPTION',
  };
}
